const compliments = [
    ...
    { text: "С тобой каждый обыденный день превращается в сказку.", icon: "✨" },
    { text: "Твоя энергия заряжает, как сияние звёзд.", icon: "⚡" },
    { text: "Ты - самое нежное и прекрасное создание во всей галактике.", icon: "🌸" },
    { text: "Твоя улыбка способна растопить самый холодный лёд.", icon: "😊" },
    { text: "Ты - моя любимая Даша, и я ценю тебя больше всего на свете!", icon: "💖" },
];

const additionalCompliments = [
    { text: "Ты - моё самое заветное желание, воплощённое в жизнь.", icon: "🌠" },
...
function createFloatingHeart() {
    ...
    heart.style.animationDuration = Math.random() * (ANIMATION_DURATIONS.heartFloat.max - ANIMATION_DURATIONS.heartFloat.min) + ANIMATION_DURATIONS.heartFloat.min + "s";
...
function createFallingCat() {
    ...
    cat.style.animationDuration = Math.random() * (ANIMATION_DURATIONS.catFall.max - ANIMATION_DURATIONS.catFall.min) + ANIMATION_DURATIONS.catFall.min + "s";
...
function renderCompliments(complimentsArray = initialCompliments) {
    ...
    complimentsArray.forEach((compliment, index) => {
        ...
        ], { duration: ANIMATION_DURATIONS.complimentHide + 100, easing: 'ease-out', delay: index * 60, fill: 'forwards' });
    });
}
...
function hideCompliments() {
    ...
    cards.forEach((card, index) => {
        const animation = card.animate([
            { opacity: 1, transform: 'scale(1) translateY(0px)' },
            { opacity: 0, transform: 'scale(0.8) translateY(-20px)' }
        ], { duration: ANIMATION_DURATIONS.complimentHide, easing: 'ease-in', delay: index * 40, fill: 'forwards' });
...
function createGlitterExplosion(x, y) {
    for (let i = 0; i < 50; i++) {
        setTimeout(() => {
            ...
            glitter.style.textShadow = `0 0 8px hsl(${hue}, 100%, 70%), 0 0 15px rgba(255,255,255,0.5)`;            glitter.style.willChange = 'transform, opacity';

            document.body.appendChild(glitter);

            glitter.animate([
                { transform: 'translate(-50%, -50%) scale(0) rotate(0deg)', opacity: 1 },
                { transform: `translate(calc(-50% + ${Math.cos((i / 50) * Math.PI * 2) * (Math.random() * 300 + 100)}px), calc(-50% + ${Math.sin((i / 50) * Math.PI * 2) * (Math.random() * 300 + 100)}px)) scale(1.5) rotate(${Math.random() * 720}deg)`, opacity: 0 }
            ], {
...
function createMagicRings(x, y) {
    for (let i = 0; i < 3; i++) { // Уменьшено количество колец
        setTimeout(() => {
            ...
            const size = 150 + i * 50; // Больший размер
            ring.animate([
                { width: '0px', height: '0px', opacity: 1, transform: 'translate(-50%, -50%) scale(0.3)' },
                { width: `${size}px`, height: `${size}px`, opacity: 0, transform: 'translate(-50%, -50%) scale(1)' }
            ], {
...
function showMessage(message, isSpecial = false) {
    ...
    if (!msgDiv || !msgP) {
        ...
        // Повторный вызов, чтобы использовать уже созданные элементы
    ...
    msgDiv.style.pointerEvents = 'auto'; // Включаем события мыши

    const animProps = isSpecial 
        ? {
            padding: "35px 60px",
            background: "linear-gradient(45deg, #FF69B4, #EE82EE, #8A2BE2)",
            boxShadow: "0 0 120px rgba(255,105,180,0.9), 0 0 20px rgba(255,255,255,0.9)",
            ...
            boxShadow: "0 25px 50px rgba(255,105,180,0.6), 0 0 10px rgba(255,255,255,0.6)",
    ...
    msgDiv.animate([
        { opacity: 0, transform: 'translate(-50%, -50%) scale(0.7) rotateX(45deg)' },
        { opacity: 1, transform: 'translate(-50%, -50%) scale(1.05) rotateX(-5deg)' },
    ...
    setTimeout(() => {
        msgDiv.animate([
            ...
            { opacity: 0, transform: 'translate(-50%, -50%) scale(1.3) rotateX(45deg)' }
...
function createGoldenRain() {
    ...
    goldenRainInterval = setInterval(() => {
        if (Math.random() > 0.9) { // Снижаем частоту
            for (let i = 0; i < 10; i++) { // Уменьшаем количество капель
                setTimeout(() => {
                    const drop = document.createElement("div");
                    drop.innerHTML = ["✨", "💫", "⭐", "🌟", "💛"][ // Убрана монета
                        Math.floor(Math.random() * 5)
                    ];
                    drop.style.position = "fixed";
                    drop.style.left = Math.random() * 100 + "%";
                    drop.style.top = "-50px";
                    drop.style.fontSize = Math.random() * 20 + 15 + "px"; // Меньше размер
                    ...
                    const duration = Math.random() *
                        (ANIMATION_DURATIONS.goldenRain.max - ANIMATION_DURATIONS.goldenRain.min) +
                        ANIMATION_DURATIONS.goldenRain.min;

                    drop.animate([
                        { top: '-50px', transform: 'rotate(0deg) scale(0.7)', opacity: 1 },
                        { top: `${window.innerHeight + 50}px`, transform: `rotate(${Math.random() * 360}deg) scale(1.2)`, opacity: 0 } // Меньше вращения
                    ], {
...
function createRotatingHearts() {
    // Эта функция будет вызываться только при клике на комплимент или кнопку
}
...
function setupMagneticButton() {
    ...
    document.addEventListener("mousemove", (e) => {
        ...
        if (distance < ANIMATION_DELAYS.magneticButtonDistance) {
            const strength = Math.max(0, (ANIMATION_DELAYS.magneticButtonDistance - distance)) * ANIMATION_DELAYS.magneticButtonStrength;
            const moveX = (e.clientX - buttonCenter.x) * (strength / ANIMATION_DELAYS.magneticButtonDistance);
            const moveY = (e.clientY - buttonCenter.y) * (strength / ANIMATION_DELAYS.magneticButtonDistance);
            
            button.style.transition = 'transform 0.08s ease-out, box-shadow 0.08s ease-out';
            ...
            if (button.style.transform !== '' || button.style.boxShadow !== '') {
...
function createCatPawPrints() {
    document.addEventListener("click", (e) => {
        ...
        for (let i = 0; i < 2; i++) { // Уменьшено количество следов
            setTimeout(() => {
                ...
                paw.animate([
                    ...
                    { opacity: 0, transform: 'scale(1.5) rotate(20deg)' } // Меньше масштаб и вращение
                ], {
                    duration: 1000,
...
function createFlowerRain() {
    ...
    flowerRainInterval = setInterval(() => {
        if (Math.random() > 0.95) { // Очень редко
            for (let i = 0; i < 5; i++) { // Очень мало цветов
                setTimeout(() => {
                    const flower = document.createElement("div");
                    flower.innerHTML = ["🌸", "🌼", "🌷"][ // Меньше видов цветов
                    ...
                    flower.style.filter = "drop-shadow(0 0 5px rgba(255,182,193,0.5))";
                    ...
                    const duration = Math.random() *
                        (ANIMATION_DURATIONS.flowerRain.max - ANIMATION_DURATIONS.flowerRain.min) +
                        ANIMATION_DURATIONS.flowerRain.min;

                    flower.animate([
                        { top: '-50px', transform: 'rotate(0deg) scale(0.7)', opacity: 1 },
                        { top: `${window.innerHeight + 50}px`, transform: `rotate(${Math.random() * 360}deg) scale(1.0)`, opacity: 0 }
                    ], {
                        duration: duration * 1000,
                        easing: 'linear',
                        fill: 'forwards'
...
function createFireworks(x, y) {
    const fireworkColors = ["#FF4500", "#FFD700", "#ADFF2F", "#1E90FF", "#DA70D6", "#FF69B4"];

    for (let i = 0; i < 2; i++) { // Уменьшено количество фейерверков
        setTimeout(() => {
            ...
            firework.animate([
                ...
                { opacity: 1, transform: `translate(${targetX - startX}px, ${targetY - startY}px) scale(1)` }
            ], {
                ...
                firework.remove();
                for (let j = 0; j < 40; j++) { // Уменьшено количество частиц
                    ...
                    const angle = (j / 40) * Math.PI * 2 + (Math.random() - 0.5) * 0.4;
                    ...
                    particle.animate([
                        ...
                        { transform: `translate(${px}px, ${py}px) scale(0.1)`, opacity: 0 }
                    ], {
...
function setupRainbowTrail() {
    ...
    const activateTrail = () => {
        ...
        removeTimeout = setTimeout(() => {
            ...
            // Удаляем зависшие частицы
            trailElements.forEach(el => el.remove());
            trailElements = [];
        }, 8000); // След активен 8 секунд
    };
    ...
    const createTrailParticle = (e) => {
        ...
        particle.animate([
            ...
            duration: 800, // Меньше продолжительность
            easing: 'ease-out',
            fill: 'forwards'
        }).onfinish = () => {
            particle.remove();
...
function createWaveEffect(card, event) {
    ...
    ripple.animate([
        ...
        { width: '150px', height: '150px', opacity: 0 } // Изменён размер волны
    ], {
        duration: 700, // Уменьшена длительность
        easing: 'ease-out',
        fill: 'forwards'
    }).onfinish = () => ripple.remove();
}

// Новая функция для эффекта звездного взрыва
...
function handleCardClick(e, card, compliment) {
    ...
    createStarExplosion(e.clientX, e.clientY); // Добавляем звездный взрыв

    card.animate([
        { transform: 'scale(1)', boxShadow: '' },
        { transform: 'scale(1.1)', boxShadow: '0 0 30px rgba(255, 105, 180, 0.7), 0 0 10px rgba(255,255,255,0.6)', backgroundColor: '#fff0f8' },
        { transform: 'scale(1)', boxShadow: '', backgroundColor: '' }
    ], { duration: ANIMATION_DURATIONS.cardClickScale, easing: 'cubic-bezier(0.175, 0.885, 0.32, 1.275)' });

    showMessage(`💖 ${compliment.text} 💖`, true); // Делаем сообщение всегда "особенным"
}
...
function setupSuperMagicButton() {
    ...
    button.addEventListener("click", (e) => {
        ...
        // Уменьшаем количество фоновых анимаций при клике
        setTimeout(() => {
            for (let i = 0; i < 10; i++) { // Меньше сердец
                setTimeout(createFloatingHeart, i * 150);
            }
        }, 200);

        setTimeout(() => {
            for (let i = 0; i < 5; i++) { // Меньше котов
                setTimeout(createFallingCat, i * 200);
            }
        }, 400);

        const initialBodyBackground = "linear-gradient(180deg, #0f0f23 0%, #1a0a1a 50%, #0f0f23 100%)";
        document.body.animate([
            { background: initialBodyBackground },
            { background: 'linear-gradient(145deg, #FF69B4, #EE82EE, #8A2BE2)' },
            { background: initialBodyBackground }
        ], { duration: ANIMATION_DURATIONS.buttonBgChange, easing: 'cubic-bezier(0.19, 1, 0.22, 1)', fill: 'forwards' });
...
window.addEventListener("load", () => {
    // Создаем контейнеры animations, если их нет
    ...
    // Создаем контейнеры для всплывающих сообщений
    ...
    if (goldenRainInterval) clearInterval(goldenRainInterval);
    if (flowerRainInterval) clearInterval(flowerRainInterval);
    rotatingHeartsIntervals.forEach(clearInterval); // Очищаем все интервалы
    rotatingHeartsIntervals = [];


    // Сокращаем частоту фоновых анимаций
    floatingHeartInterval = setInterval(createFloatingHeart, ANIMATION_DELAYS.heartFloat * 3);
    fallingCatInterval = setInterval(createFallingCat, ANIMATION_DELAYS.catFall * 5);
    starInterval = setInterval(createStar, ANIMATION_DELAYS.sparkleCreation * 100); // Реже
    goldenRainInterval = setInterval(createGoldenRain, 3000); // Реже
    flowerRainInterval = setInterval(createFlowerRain, 8000); // Очень редко

    // createRotatingHearts() не будет вызываться постоянно

    setupMagneticButton();
    createCatPawPrints(); // Можно оставить, так как она по клику
